...
switch current_sub_panel
    ...
    case 'format'
        ...
        if selected_src_format_typ ==1
            ...
            adv_pre_proc_button_list=[{{'style','text','string',...
                'If behavioral coding was used, please fill out either Section 1 or 2, depending on the type of behavioral coding used, and leave the other section blank. Additionally, please complete Section 3 as well.','FontWeight', 'bold'}},...
                {{'style','text','string', 'If behavioral coding was not used, skip to section 3.','FontWeight', 'bold'}},...
                extra_space_line,...
                {{'style','text','string', 'SECTION 1: Behavioral Coding based on the closest behavioral code after an event of interest','FontWeight', 'bold'}},...
                {{'style','text','string', ...
                ...
                extra_space_line,...
                {{'style','text','string', 'SECTION 2: Behavioral Coding based on start and end markers','FontWeight', 'bold'}},...
                {{'style','text','string', 'The tag for when artifact started, ex. VST_'}},...
                {{'style','edit','string', grp_proc_info.flag_for_bad_value_start_end{1} , 'tag','bad_val_start'}},...
                {{'style','text','string', 'The tag for when artifact started, ex. VSE_'}},...
                {{'style','edit','string', grp_proc_info.flag_for_bad_value_start_end{2} , 'tag','bad_val_end'}},...
                {{'style','text','string', 'A string that marks trials as bad, can be any user-defined string, ex. badt'}},...
                {{'style','edit','string', grp_proc_info.behavioral_coding.bad_value , 'tag','bad_val'}},...
                extra_space_line,...
                {{'style','text','string', 'SECTION 3: Recording Periods to Analyze (ex. [1:3,4,6] def: []; to process all periods): ','FontWeight', 'bold'}},...
            ...
            adv_pre_proc_button_geometry = {1 1 1 1 1 1 1 1 [.7 .3] [.7 .3] [.7 .3] 1 [.7 .3]}; %MS added two more fields for (1) bad val start and (2) bad val end
            adv_pre_proc_ver_geometry = [1 1 1 1 1 6 1 1 1 1 1 1 1]; %MS added two more fields for (1) bad val start and (2) bad val end
        end
